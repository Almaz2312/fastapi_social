...
async def get_profile(current_user, db: Session):
    profile = db.query(User).filter(User.id == current_user.id).first()
    return profile


async def follow_user(follow: Follow_Schema, user_id: int, db: Session):
